...
setGlobalOptions({ region: "australia-southeast1", maxInstances: 40 });
...
const FIRESTORE_DATABASE_ID =
  process.env.FIRESTORE_DATABASE_ID?.trim() || "gsmsimcared";
const db = getFirestore(
  admin.app(),
  FIRESTORE_DATABASE_ID === "" ? "(default)" : FIRESTORE_DATABASE_ID
);
...
const SMS_RETRY_DELAYS_MS = [30_000, 120_000, 600_000] as const;
export const THOMASTOWN_WELCOME_TEMPLATE =
  "Hi {name}, gate access is active at 337 Settlement Rd, Thomastown. Call 0414 371 302 to open. Caller ID must be visible. Help: 0400 101 132.";
export const SMS_MULTIPART_SAFE_CHARS = 160;
...
export const requestRemoteGateTest = onCall(async (request) => {
  ...
  if (!profile.exists || user?.enabled !== true || user?.role !== "OWNER") {
    throw new HttpsError("permission-denied", "Only the property owner can run a gate test.");
  }
...
export const askPortalAssistant = onCall(
  { secrets: [XAI_API_KEY], timeoutSeconds: 90 },
  async (request) => {
    const message = String(request.data?.message || "").trim().slice(0, 2_000);
    const pageContext = String(request.data?.pageContext || "").trim().slice(0, 1_000);
    if (!message) throw new HttpsError("invalid-argument", "Enter a question.");

    const rawHistory = Array.isArray(request.data?.history) ? request.data.history : [];
    const history = rawHistory.slice(-12).flatMap((item: unknown) => {
    ...
    if (!reply) throw new HttpsError("unavailable", "The AI assistant returned no reply.");
    return { reply };
  }
);
 ...
 */
export const createSmsCampaign = onCall(async (request) => {
  const uid = request.auth?.uid;
  if (!uid) throw new HttpsError("unauthenticated", "Sign in is required.");
  ...
  if (
    ...
    throw new HttpsError("permission-denied", "Only the property owner can send SMS.");
  }

  const accountId = String(profileData.accountId || "");
  const message = String(request.data?.message || "").trim();
  const callerIds: string[] = Array.isArray(request.data?.callerIds)
    ? Array.from(
        new Set<string>(
          (request.data.callerIds as unknown[]).map((id: unknown) => String(id || ""))
        )
      ).filter((id) => id.length > 0)
    : [];
  if (!accountId) throw new HttpsError("failed-precondition", "No property is assigned.");
  if (!message || message.length > SMS_MULTIPART_SAFE_CHARS) {
    throw new HttpsError(
      "invalid-argument",
      `Message must be 1-${SMS_MULTIPART_SAFE_CHARS} characters.`
    );
  }
  if (callerIds.length < 1 || callerIds.length > 200) {
    throw new HttpsError("invalid-argument", "Choose between 1 and 200 recipients.");
  }
  ...
  const [account, devices, ...callerDocs] = await Promise.all([
    ...
    ...callerIds.map((id: string) => accountRef.collection("gsmCallers").doc(id).get()),
  ]);
  if (!account.exists || account.data()?.enabled === false) {
    throw new HttpsError("failed-precondition", "This property is disabled.");
  }
  const deviceId = devices.docs[0]?.id || "device_commercial_bc_01";
  const now = Date.now();
  const recipients = callerDocs.flatMap((snap) => {
    ...
    if (!isCallerActive({ id: snap.id, ...data }, now)) return [];
  ...
  if (!recipients.length) {
    throw new HttpsError("failed-precondition", "No selected recipients are currently active.");
  }
...
async function loadDeviceCred(deviceId: string): Promise<DeviceCred | null> {
...
function verifySecret(deviceId: string, secret: string, expectedHash: string): boolean {
...
async function claimNonce(deviceId: string, nonce: string): Promise<boolean> {
  ...
  try {
    await db.runTransaction(async (tx) => {
      ...
      if (snap.exists) throw new Error("reuse");
      tx.set(ref, {
        deviceId,
        nonce,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
...
function isCallerActive(c: FirebaseFirestore.DocumentData, now: number): boolean {
  if (c.enabled === false) return false;
  const from = c.validFrom?.toMillis?.() ?? c.validFrom ?? null;
  const until = c.validUntil?.toMillis?.() ?? c.validUntil ?? null;
  if (from != null && now < Number(from)) return false;
  if (until != null && now > Number(until)) return false;
  const phone = String(c.phoneNumberE164 || "");
  if (!phone.startsWith("+") || phone.length < 9) return false;
  return true;
}
...
function normalizeAustralianMobile(raw: string): string | null {
  ...
  if (/^04\d{8}$/.test(digits)) digits = `61${digits.slice(1)}`;
  else if (/^4\d{8}$/.test(digits)) digits = `61${digits}`;
  if (!/^614\d{8}$/.test(digits)) return null;
  return `+${digits}`;
}

/** Build exactly one GSM-7 part, shortening a long/compound name if required. */
export function buildWelcomeSmsParts(displayName: string): string[] {
  const usable = displayName.trim().match(/[A-Za-z][A-Za-z'-]*/g) || [];
  if (!usable.length) return [];
  const candidates = [usable[0]!, displayName.trim()];
  for (const name of candidates) {
    const message = THOMASTOWN_WELCOME_TEMPLATE.replace("{name}", name);
    if (/^[\x0A\x0D\x20-\x7E]*$/.test(message) &&
        message.length <= SMS_MULTIPART_SAFE_CHARS) return [message];
  }
  return [];
}
...
export const gsmDeviceApi = onRequest(
  {
    cors: false,
    invoker: "public",
  },
  async (req, res) => {
    try {
      ...
      if (
        !Number.isFinite(secretVersionClient) ||
      ...
      const sigOk =
        timingSafeEqual(expectedSig, signature.toLowerCase()) ||
      ...
      if (action === "whitelist" && req.method === "GET") {
        ...
        const gateSim = String(
          accountSnap.data()?.gsmNumber || accountSnap.data()?.gsmNumberE164 || ""
        );
        ...
        const callers = callersSnap.docs
          ...
          .map((c) => {
            const e164 = normalizeAustralianMobile(
              String(c.data.phoneNumberE164 || c.data.phoneNumber || "")
            );
            if (!e164) return null;
            return {
              id: c.id,
        ...
        let smsJobs: Array<Record<string, unknown>> = [];
        try {
          const all = await db
          ...
          eligibleJobs.sort((a, b) =>
            String(a.callerId).localeCompare(String(b.callerId)) ||
        ...
        let remoteCommands: Array<Record<string, unknown>> = [];
        try {
      ...
      if (action === "whitelist-ack" && req.method === "POST") {
        ...
        if (
          !Number.isSafeInteger(appliedVersion) ||
          appliedVersion < 0 ||
        ...
        await db.collection("clientActionLogs").add({
          ...
          detail:
            `v=${appliedVersion} cloud=${cloudVersion} callers=${callerCount} ` +
          ...
          if (ok) {
            await db.runTransaction(async (tx) => {
              tx.set(jobRef, {
                ...
                lastError: null,
                modemMessageReference:
                  r.cmgsReference != null ? String(r.cmgsReference) : null,
              }, { merge: true });
              const campaignKey = String(job.campaignKey || "");
              if (job.callerId && Number(job.partIndex || 0) + 1 >= Number(job.partCount || 1)) {
                tx.set(
                  accountRef.collection("gsmCallers").doc(String(job.callerId)),
                  {
            ...
            const retryDelay =
              SMS_RETRY_DELAYS_MS[Math.min(attempts - 1, SMS_RETRY_DELAYS_MS.length - 1)];
            await jobRef.set(
              {
                status,
                attemptCount: attempts,
        ...
        for (const id of jobIds) {
          ...
          try {
            const claimed = await db.runTransaction(async (tx) => {
              ...
              const job = snap.data()!;
              if (job.accountId !== accountId || job.deviceId !== deviceId) return false;
              ...
              if (
                (job.status !== "queued" && !staleSending) ||
            ...
            if (claimed) claimedJobIds.push(jobId);
          } catch (e: any) {
      ...
      if (action === "command-claim" && req.method === "POST") {
        ...
        if (!commandId) return json(res, 400, { error: "command_id_required" });
      ...
      if (action === "events" && req.method === "POST") {
        ...
        const signalStrength =
          body.signalStrength != null ? Number(body.signalStrength) : null;
        ...
        const payload = {
          ...
          ...(validLocation
            ? {
                ...
                gnssAltitudeMetres:
                  rawLocation.altitudeMetres != null
                    ? Number(rawLocation.altitudeMetres)
                    : null,
                gnssSpeedKnots:
                  rawLocation.speedKnots != null
                    ? Number(rawLocation.speedKnots)
                    : null,
                gnssHeadingDegrees:
                  rawLocation.headingDegrees != null
                    ? Number(rawLocation.headingDegrees)
                    : null,
                gnssCapturedAt:
                  Number(rawLocation.capturedAtEpoch) > 0
                    ? admin.firestore.Timestamp.fromMillis(
                        Number(rawLocation.capturedAtEpoch) * 1000
                      )
                    : admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
export const queueWelcomeSmsOnCallerCreated = onDocumentCreated(
  {
  ...
  async (event) => {
    ...
    if (!caller || caller.enabled === false) return;
    ...
    const phoneNumberE164 = normalizeAustralianMobile(
      String(caller.phoneNumberE164 || caller.phoneNumber || "")
    );
    if (
      !displayName ||
      !phoneNumberE164 ||
    ...
    if (!siteText.includes("thomastown")) return;
 ...
 */
export async function provisionDeviceCredential(params: {
  deviceId: string;
  accountId: string;
  secret: string;
  secretVersion?: number;
